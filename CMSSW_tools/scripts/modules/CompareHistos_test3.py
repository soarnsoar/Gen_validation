...
def CompareTGraphAsymmErrorsWithStatMutiNumerators(xaxis,yaxis,yaxis_ratio,title,h__sys,h__stat,labels,colors,store_file_path,do_norm=True):
    ##---h_deno:sys
    ##--h_deno_stat:stat
    ...
    print '[label denominator]',labels[0]
    print '[labels]'
    ...
    for ih in range(0,n_hist):
        h__total.append(h__sys[ih].Clone(labels[ih]+'__total'))
        h__stat .append(h__stat[ih].Clone(labels[ih]+'__stat' ))
        h__sys  .append(h__sys[ih].Clone(labels[ih]+'__sys'  ))
        h__ratio__total.append(h__sys[ih].Clone(labels[ih]+'__ratio__total'))
        h__ratio__stat .append(h__stat[ih].Clone(labels[ih]+'__ratio__stat '))
        h__ratio__sys  .append(h__sys[ih].Clone(labels[ih]+'__ratio__sys  '))
        
        ##---sys err percent
        tgr__syserr.append( h__sys[ih].Clone(labels[ih]+'__syserr') )
        ysum.append( sum(   h__sys[ih].GetY()   ) )
        if do_norm:
            norm.append(ysum[ih]/ysum[0])
        else:
            norm.append(1)
    ...
    ##--for plot range
    ymax  =max(h__sys[0].GetY())/norm[0]
    ymin  =max(h__sys[0].GetY())/norm[0]
    syserrmax=0
    ...
    canvas_name=prefix+'__'+"_over_"+labels[0]+'__'+xaxis+'__'+title
    tcanvas = ROOT.TCanvas(canvas_name,canvas_name,800,800)
    #--only error comparison
    canvas_name2=prefix+'__'+"_over_"+labels[0]+'__'+xaxis+'__'+title+'__percent_error'
...
if __name__ == "__main__":
    ...
    xaxis="dimuon_mass"
    yaxis='norm. nevents'
    yaxis_ratio='mg261/mg260'
    title='muRmuF_default'
    store_file_path='./result'
    h__sys=[]
    h__sys.append(h2)
    h__sys.append(h1)
    h__stat=[]
    ...
    h__stat.append(h1)
    labels=[]
    labels.append('MG5 v260')
    labels.append('MG5 v261')
    colors=[]
    colors.append(ROOT.kBlue)
    colors.append(ROOT.kRed)

    CompareTGraphAsymmErrorsWithStatMutiNumerators(xaxis,yaxis,yaxis_ratio,title,h__sys,h__stat,labels,colors,store_file_path,do_norm=True)
