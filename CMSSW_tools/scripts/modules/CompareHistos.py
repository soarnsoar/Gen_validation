...
def CompareTGraphAsymmErrorsWithStat(xaxis,yaxis,yaxis_ratio,title,h1,h1stat,label1,color1,h2,h2stat,label2,color2,store_file_path,do_norm=True):
    ...
    if h1.GetN()!=h2.GetN():
        print "[CompareHistos.py] two histograms do not have the same size"
        exit()
    ...
    ##--Legend
    tlegend = ROOT.TLegend(0.50, 0.65, 0.88, 0.88)#x1,y1,x2,y2 
    ...
    tlegend.AddEntry(h1__total,label1)
    tlegend.AddEntry(h2__total,label2)

    tlegend.AddEntry(h1,'Sys.Err.')
    tlegend.AddEntry(h2,'Sys.Err.')

    tlegend.AddEntry(h1stat, ' Stat.Err.')
    tlegend.AddEntry(h2stat, ' Stat.Err.')
    ...
    tlegend2.AddEntry(tgr_syserr1,label1)
    tlegend2.AddEntry(tgr_syserr2,label2)
    ...
    sys_tmr.SetMaximum(syserrmax*2)
    sys_tmr.SetMinimum(-syserrmax)
    tlegend2.Draw()
...
if __name__ == "__main__":
    ...
    h2path='/cms/ldap_home/jhchoi/gridvalidation/mg265_validation/event_gen/workdir/JOBDIR_HistoFactory__GENEVT_mg261_dyellell01234j_5f_LO_MLM__5000evt/combined_histo/'
    ...
    CompareTGraphAsymmErrorsWithStat("dimuon_mass",'norm. nevents','mg260/mg261','LHAPDF306000', h1, h1stat,"MG5 v260",ROOT.kRed,h2,h2stat,"MG5 v261",ROOT.kBlue,'./result.pdf',True)
