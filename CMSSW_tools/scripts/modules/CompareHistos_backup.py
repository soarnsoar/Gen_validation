...
def ErrorCombineSqrtSumTGraphAsymmErrors(h1,h2):
    ...
    h__combine=TGraphAsymmErrors(nbin)
...
def ScaleTGraphAsymmErrors(h,scale):
    ...
    for iBin in range(0,nbin):
        ...
        h.SetPointError(iBin, GetErrorXlow(iBin),GetErrorXhigh(iBin), scale*GetErrorYlow(iBin), scale*GetErrorXlow(iBin)   )
...
def GetRatioTGraphAsymmErrors(h1,h2):
    ...
    tgr_h1_over_h2=TGraphAsymmErrors(nbin)
...
def CompareTGraphAsymmErrorsWithStat(xaxis,yaxis,yaxis_ratio,title,h1,h1stat,label1,color1,h2,h2stat,label2,color2,store_file_path,test_stat,pvalue_threshold,do_norm=True):
    ...
    del th1d_stat2
    '''
    stat_test_color=ROOT.kRed
    if pass_stat_central:
        stat_test_color=ROOT.kBlue
    ...
    tbox=ROOT.TLegend(0.01,0.01,0.99,0.99) #TBox (Double_t x1, Double_t y1, Double_t x2, Double_t y2)
    tbox.SetLineWidth(3)
    tbox.SetLineColor(stat_test_color)
    tbox.SetFillStyle(0)                                                                                                         
    ...
    tbox.Draw()
    tcanvas.Update()
    tcanvas.SaveAs(store_file_path+'__'+test_stat+'.pdf')
...
if __name__ == "__main__":
    ...
    h1=ROOT.GetTGraphAsymmErrors(h1path+'/combined_histo__muRmuF_default__dimuon_mass_60_120.root',"Graph")
    h2=ROOT.GetTGraphAsymmErrors(h2path+'/combined_histo__muRmuF_default__dimuon_mass_60_120.root',"Graph")
    h1stat=ROOT.GetTGraphAsymmErrors(h1path+'/combined_histo__muRmuF_default__dimuon_mass_60_120__statonly.root',"Graph")
    h2stat=ROOT.GetTGraphAsymmErrors(h2path+'/combined_histo__muRmuF_default__dimuon_mass_60_120__statonly.root',"Graph")
